...
export default function StatusModal() {
  ...
  return (
    <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 font-sans text-left">
      {/* Backdrop */}
      <div 
        className="absolute inset-0 bg-bk-main/60 backdrop-blur-md animate-in fade-in duration-300"
        onClick={() => dispatch(closeStatusModal())}
      ></div>
      
      {/* Modal Content */}
      <div className="relative bg-white dark:bg-bk-side rounded-xl shadow-[0_20px_60px_rgba(0,0,0,0.4)] w-full max-w-[380px] border border-slate-200 dark:border-slate-800 overflow-hidden transform transition-all z-10 p-7 flex flex-col items-center text-center animate-in zoom-in-95 duration-300">
        ...
        {/* Icon Container */}
        <div className={`w-16 h-16 ${currentTheme.bg} ${currentTheme.border} border rounded-2xl flex items-center justify-center mb-5 rotate-3 animate-in zoom-in duration-500`}>
