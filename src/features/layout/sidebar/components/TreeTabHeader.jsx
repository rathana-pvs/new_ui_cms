...
export default function TreeTabHeader({ activeTab, setActiveTab, onDbTabContextMenu, onBrokerTabContextMenu }) {
  ...
  return (
    <div className="flex items-center gap-0.5 px-2 py-2 border-b border-slate-100 dark:border-white/[0.05] bg-white dark:bg-background-dark shrink-0">
      {tabs.map((tab) => {
        const isActive = activeTab === tab.id;
        return (
          <button
            key={tab.id}
            className={`flex-1 flex items-center justify-center gap-1.5 py-1.5 px-2 rounded-md text-[11px] font-semibold tracking-wide transition-all duration-150 select-none relative
              ${isActive
                ? 'bg-amber-500/10 text-amber-600 dark:text-amber-400'
                : 'text-slate-400 dark:text-slate-500 hover:text-slate-600 dark:hover:text-slate-300 hover:bg-slate-100 dark:hover:bg-white/[0.04]'
              }`}
