...
export default function AutoBackupLogModal() {
  ...
  // Filter by database if one was selected during context menu trigger
  // And also by search term
    ...
    <div className="fixed inset-0 z-[150] flex items-center justify-center p-4 bg-bk-main/40 backdrop-blur-sm animate-in fade-in duration-200 font-sans text-left">
      <div className="bg-white dark:bg-bk-side w-full max-w-[800px] h-[600px] rounded-[6px] shadow-[0_10px_40px_rgba(0,0,0,0.3)] border border-slate-200 dark:border-slate-800 overflow-hidden animate-in zoom-in-95 duration-200 flex flex-col relative text-left">
        ...
        <LoadingOverlay 
          ...
          title="Fetching backup logs" 
          subtitle="Retrieving historical backup error data from server..." 
        />
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-bk-main/50 flex-shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-[14px] font-semibold text-slate-900 dark:text-white leading-none tracking-tight">Auto Backup Log</h3>
              <p className="text-[10px] text-slate-500 dark:text-slate-400 mt-1.5 font-medium uppercase tracking-wider">
                {selectedDatabase ? `History for database: ${selectedDatabase}` : 'Global Backup History'}
              </p>
            </div>
          </div>
        ...
        <div className="flex-1 overflow-hidden flex flex-col bg-white dark:bg-bk-side">
          <div className="px-6 py-4 flex flex-col gap-3">
             <div className="text-[13px] font-medium text-slate-600 dark:text-slate-400">
               Below you can view the unload database result
             </div>
          ...
          {logsError ? (
            ...
            <div className="flex-1 flex flex-col items-center justify-center p-12 text-center opacity-60">
              ...
              <h4 className="text-[14px] font-medium text-slate-800 dark:text-white mb-1">No backup logs found</h4>
              ...
              <div className="border border-slate-100 dark:border-slate-800 rounded-[6px] overflow-hidden">
                <table className="w-full border-collapse">
                  <thead className="bg-slate-50/90 dark:bg-bk-main/90 backdrop-blur-sm">
                    <tr>
                      <th className="px-5 py-3 text-left text-[11px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider border-b border-slate-100 dark:border-slate-800">Backup ID</th>
                      <th className="px-5 py-3 text-left text-[11px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider border-b border-slate-100 dark:border-slate-800">Log Time</th>
                      <th className="px-5 py-3 text-left text-[11px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider border-b border-slate-100 dark:border-slate-800">Description</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-slate-100 dark:divide-slate-800/50">
                    {filteredLogs.map((log, idx) => {
                      ...
                      return (
                        ...
                        >
                          <td className="px-5 py-4 whitespace-nowrap text-[11px] font-medium text-amber-600 dark:text-bk-yellow italic">
                            {log.backupid}
                          </td>
