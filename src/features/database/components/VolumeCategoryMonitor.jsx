...
export default function VolumeCategoryMonitor({ hostUid, dbname, category }) {
  ...
  const formatMB = (pages) => ((parseInt(pages) * pageSize) / (1024 * 1024)).toFixed(1);
  ...
  return (
    <div className="flex-1 flex flex-col h-full bg-white dark:bg-background-dark overflow-hidden select-none animate-in fade-in duration-300">
      ...
      {/* ── Header ── */}
      <header className="px-6 py-4 border-b border-slate-100 dark:border-white/[0.04] flex items-center justify-between shrink-0 bg-white dark:bg-background-dark">
        ...
        <div className="hidden md:flex items-center gap-6">
          <div className="text-right">
            ...
            <Typography variant="p" className="text-sm font-black text-slate-700 dark:text-slate-200 font-mono leading-none">{formatSize(summary.total)}</Typography>
          </div>
        ...
        <div className="max-w-7xl mx-auto space-y-5">
          ...
          <div className="grid grid-cols-3 gap-4">
            {[
              ...
              { label: 'Used', val: formatSize(summary.used), icon: 'data_usage', color: 'text-amber-500', bg: 'bg-amber-50 dark:bg-amber-500/10', border: 'border-amber-100 dark:border-amber-500/20' },
              { label: 'Free', val: formatSize(summary.free), icon: 'check_circle', color: 'text-emerald-500', bg: 'bg-emerald-50 dark:bg-emerald-500/10', border: 'border-emerald-100 dark:border-emerald-500/20' },
            ].map((stat, i) => (
              <div key={i} className={`px-5 py-4 rounded-xl ${stat.bg} border ${stat.border} flex items-center gap-4`}>
                <div className={`w-8 h-8 rounded-lg bg-white dark:bg-black/20 flex items-center justify-center shadow-sm shrink-0 ${stat.color}`}>
          ...
          <div className="bg-white dark:bg-white/[0.02] border border-slate-200 dark:border-white/[0.05] rounded-xl px-5 py-4">
            ...
            <div className="w-full h-2 bg-slate-100 dark:bg-white/[0.05] rounded-none overflow-hidden">
              <div
            ...
            <div className="flex justify-between mt-1.5">
              <Typography variant="label" className="text-[9px] text-slate-400 font-mono">{formatSize(summary.used)} used</Typography>
              <Typography variant="label" className="text-[9px] text-slate-400 font-mono">{formatSize(summary.free)} free</Typography>
            </div>
          </div>

          {/* Volume Table */}
          <Card bodyClassName="p-0 overflow-hidden" className="border-slate-200 dark:border-white/[0.05] shadow-sm bg-white dark:bg-white/[0.01]">
            <Table
              columns={[
                {
                  header: 'Volume Name',
                  ...
                  render: (val) => {
                    ...
                    return (
                      <div className="flex items-center gap-3 py-0.5">
                        <div className="w-7 h-7 rounded bg-slate-100 dark:bg-white/[0.05] flex items-center justify-center text-slate-400 shrink-0">
                    ...
                    return (
                      <div className="flex flex-col gap-1.5 py-1 min-w-[240px]">
                        ...
                        <div className="flex justify-between">
                          <span className="text-[10px] font-bold font-mono text-slate-500">{formatMB(usedPages)} MB used</span>
                ...
                },
                {
                  ...
                  render: (val) => (
                    <span className="text-[13px] font-bold text-slate-600 dark:text-slate-300 font-mono">{formatSize(val)}</span>
                  )
                },
                {
                  header: 'Pages',
      ...
      {/* ── Footer ── */}
      <footer className="px-6 py-3 border-t border-slate-100 dark:border-white/[0.04] flex items-center justify-between shrink-0">
