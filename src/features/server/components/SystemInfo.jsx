...
export default function SystemInfo({ hostUid }) {
  ...
  const InfoRow = ({ label, value }) => (
    <div className="flex items-start py-1.5 border-b border-slate-100/50 dark:border-slate-800/30 last:border-0">
      <span className="text-slate-500 font-medium text-[12px] tracking-wide font-sans min-w-[120px] uppercase">{label}</span>
      <span className="text-slate-900 dark:text-slate-100 font-mono text-[13px] break-all">{value || 'N/A'}</span>
    </div>
  );
