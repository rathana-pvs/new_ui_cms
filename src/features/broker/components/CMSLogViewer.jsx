...
function CMSLogViewer({ hostUid, type }) {
  ...
  return (
    <div className="flex-1 flex flex-col bg-slate-50 dark:bg-bk-main overflow-hidden font-mono">
      ...
      {/* Table Header */}
      <div className="flex-shrink-0 flex items-center gap-4 px-4 py-2 bg-slate-100 dark:bg-white/5 border-b border-slate-200 dark:border-slate-800 text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest">
        <div className="w-36 flex-shrink-0">Time</div>
        <div className="w-24 flex-shrink-0">User</div>
        ...
        {!paginatedLogs || paginatedLogs.length === 0 ? (
          ...
          <div className="min-w-full inline-block">
            {paginatedLogs.map((log, idx) => (
              ...
              >
                <div className="w-36 flex-shrink-0 text-slate-500 group-hover:text-slate-600 dark:group-hover:text-slate-400 transition-colors">
                  {log.time}
                </div>
